...
use clap::Parser;
use code_graph_flattener::{CodeGraph, GraphEdge, GraphNode};
...
fn main() -> Result<()> {
    ...
    let graph_content = fs::read_to_string(&args.graph_path).context(format!(
        "Failed to read CodeGraph file from {}",
    ...
    println!(
        "CodeGraph loaded with {} nodes and {} edges.",
        code_graph.nodes.len(),
        code_graph.edges.len()
    );

    let collected_analysis_data: Option<CollectedAnalysisData> =
        if let Some(path) = &args.analysis_data_path {
            println!("Loading CollectedAnalysisData from: {}", path.display());
            let analysis_data_content = fs::read_to_string(path).context(format!(
                "Failed to read CollectedAnalysisData file from {}",
                path.display()
            ))?;
            Some(
                serde_json::from_str(&analysis_data_content)
                            ...
                            if let Some(captures) = command_new_regex.captures(expr_str) {
                                if let Some(name) = captures.get(1) {
                                    ...
                                    if name.as_str().starts_with("./")
                                        || name.as_str().contains("target/debug")
                                        || name.as_str().contains("target/release")
                                    {
...
",
        ...
        "type-definition-locations" => {
            report_content.push_str(
                "Type Definition Locations Report:
...
",
            ...
            for edge in &code_graph.edges {
                if let Some(type_name) = type_node_ids.get(&edge.target) {
                    let location_description = format!(
                        "  - Referenced by '{}' ({} edge)",
            ...
            if type_locations.is_empty() {
                ...
                let mut sorted_types: Vec<(&String, &Vec<String>)> =
                    type_locations.iter().collect();
        ...
        "trait-classification" => {
            report_content.push_str(
                "Trait Classification Report:

",
            );
            let analysis_data = collected_analysis_data
                .as_ref()
                .context("CollectedAnalysisData is required for 'trait-classification' query.")?;

            let mut existing_node_ids: HashSet<String> =
                code_graph.nodes.iter().map(|n| n.id.clone()).collect();
            let mut existing_edge_ids: HashSet<String> = code_graph
                .edges
                .iter()
                .map(|e| format!("{}_{}_{}", e.source, e.edge_type, e.target))
                .collect();

            // Regex to parse "Type for Trait" or "Type"
            let impl_for_trait_regex =
                Regex::new(r"^(?P<implementing_type>[^ ]+) for (?P<implemented_trait>.*)$")
                    .unwrap();
                ...
                if is_inherent_impl {
                    ...
                    if !existing_node_ids.contains(&inherent_trait_node_id) {
                        code_graph.nodes.push(GraphNode {
                            ...
                            properties: HashMap::from([
                                ("name".to_string(), inherent_trait_name.clone()),
                                ("kind".to_string(), "Inherent".to_string()),
                            ]),
                        });
                    ...
                    let impl_edge_id = format!(
                        "{}_implements_{}",
                        implementing_type_node_id, inherent_trait_node_id
                    );
                    ...
                    for (method_names_str, _count) in &impl_lattice.method_co_occurrences {
                        for method_name in method_names_str.split("::") {
                            let method_node_id =
                                format!("method_{}_{}", inherent_trait_node_id, method_name);
                            ...
                            let has_method_edge_id =
                                format!("{}_has_method_{}", inherent_trait_node_id, method_node_id);
                    ...
                    if !existing_node_ids.contains(&trait_node_id) {
                        code_graph.nodes.push(GraphNode {
                            ...
                            node_type: "TraitDefinition".to_string(),
                            properties: HashMap::from([
                                ("name".to_string(), trait_name.clone()),
                                ("kind".to_string(), "Explicit".to_string()),
                            ]),
                        });
                    ...
                    let impl_edge_id =
                        format!("{}_implements_{}", implementing_type_node_id, trait_node_id);
                    if !existing_edge_ids.contains(&impl_edge_id) {
                        code_graph.edges.push(GraphEdge {
                            source: implementing_type_node_id.clone(),
                    ...
                    for (method_names_str, _count) in &impl_lattice.method_co_occurrences {
                        for method_name in method_names_str.split("::") {
                            let method_node_id =
                                format!("method_{}_{}", trait_node_id, method_name);
                            if !existing_node_ids.contains(&method_node_id) {
                                code_graph.nodes.push(GraphNode {
                                    id: method_node_id.clone(),
                                    node_type: "MethodSignature".to_string(),
                            ...
                            let has_method_edge_id =
                                format!("{}_has_method_{}", trait_node_id, method_node_id);
                            if !existing_edge_ids.contains(&has_method_edge_id) {
                                code_graph.edges.push(GraphEdge {
            ...
            report_content.push_str(&format!(
                "Found {} explicit trait implementations.\n",
                trait_implementations_found
            ));
            report_content.push_str(&format!(
                "Found {} inherent implementations.\n",
            ...
            for edge in &code_graph.edges {
                if edge.edge_type == "Implements" {
                    ...
                    if trait_id.contains("_InherentMethods") {
                        let type_name = trait_id
                            .strip_prefix("trait_")
                            .unwrap_or(&trait_id)
                            .strip_suffix("_InherentMethods")
                            .unwrap_or(&trait_id)
                            .to_string();
                        // Collect methods for this inherent impl
                        let methods: Vec<String> = code_graph
                            .edges
                            .iter()
                            .filter(|e| e.source == trait_id && e.edge_type == "HasMethod")
                            .map(|e| {
                                e.target
                                    .strip_prefix(&format!("method_{}_", trait_id))
                                    .unwrap_or(&e.target)
                                    .to_string()
                            })
                            .collect();
    ...
    fs::create_dir_all(args.output_path.parent().unwrap()).context(format!(
        "Failed to create parent directory for query report: {}",
        args.output_path.display()
    ))?;
    fs::write(&args.output_path, report_content).context(format!(
        "Failed to write query report to {}",
        args.output_path.display()
    ))?;
    println!(
        "Query report successfully written to {}",
        args.output_path.display()
    );

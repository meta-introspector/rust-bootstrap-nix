...
fn main() -> Result<()> {
    ...
    println!(
        "Loading CollectedAnalysisData from: {}",
        args.input_analysis_data_path.display()
    );
    let analysis_data_content =
        fs::read_to_string(&args.input_analysis_data_path).context(format!(
    ...
    println!(
        "CodeGraph generated with {} nodes and {} edges.",
    ...
    fs::write(&args.output_graph_path, graph_json).context(format!(
        "Failed to write CodeGraph to {}",
        args.output_graph_path.display()
    ))?;

    println!(
        "CodeGraph successfully written to {}",
        args.output_graph_path.display()
    );

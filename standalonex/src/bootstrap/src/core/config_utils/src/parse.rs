...
pub fn parse(mut flags: LocalFlags) -> ParsedConfig {
    ...
    applicators.push(Box::new(
        llvm_assertions_config::LlvmAssertionsConfigApplicator,
    ));
    applicators.push(Box::new(
        rust_channel_git_hash_config::RustChannelGitHashConfigApplicator,
    ));

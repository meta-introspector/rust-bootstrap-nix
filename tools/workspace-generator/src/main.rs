...
fn main() -> anyhow::Result<()> {
    ...
    if verbosity >= 1 {
        println!(
            "Workspace generator started with verbosity level: {}",
            verbosity
        );
    }
    ...
    let external_dependencies_by_level: HashMap<u32, HashSet<String>> = if dependencies_json_path
        .exists()
    {
    ...
    println!(
        "Searching for dependency level directories in: {:?}/src",
        output_dir
    );
            ...
            if let Some(dir_name) = path.file_name().and_then(|s| s.to_str()) {
                ...
                if dir_name.starts_with("level_") {
                    if let Ok(level) = dir_name["level_".len()..].parse::<u32>() {
                        ...
                        for type_entry in WalkDir::new(&src_dir_for_package)
                            .min_depth(1)
                            .max_depth(1)
                            .into_iter()
                            .filter_map(|e| e.ok())
                        {
                            ...
                            if type_path.is_dir() {
                                if let Some(type_name_original) =
                                    type_path.file_name().and_then(|s| s.to_str())
                                {
                                    ...
                                    for decl_entry in WalkDir::new(&type_path)
                                        .min_depth(1)
                                        .max_depth(1)
                                        .into_iter()
                                        .filter_map(|e| e.ok())
                                    {
                                                ...
                                                .map_or(false, |ext| ext == "rs")
                                        {
                                            if let Some(decl_name) =
                                                decl_path.file_stem().and_then(|s| s.to_str())
                                            {
                                    ...
                                    if !mod_rs_content.is_empty() {
                                        ...
                                        println!(
                                            "  Skipping empty module for type: {}",
                                            type_name_original
                                        );
                                    }
                                }

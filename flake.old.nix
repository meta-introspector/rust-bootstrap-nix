{
  ...
  inputs = {
    ...
    standalonex.url = "path:./standalonex";
  ...
  outputs = { self, nixpkgs, rust-overlay, rustSrcFlake, flake-utils, configuration-nix, standalonex, configGenerator }:
    let
      ...
      rustToolchain_aarch64 = pkgs_aarch64.rust-bin.stable."1.84.1".default;

      pkgs_x86_64 = import nixpkgs { system = "x86_64-linux"; overlays = [ rust-overlay.overlays.default ]; };
      rustToolchain_x86_64 = pkgs_x86_64.rust-bin.stable."1.84.1".default;
      ...
      generateConfigTomlForStage = { system, pkgs, rustToolchain, configurationNix, stageNum }:
        pkgs.writeText "config-stage-${toString stageNum}.toml" ''
          [build]
    ...
    in
    {
      packages.aarch64-linux = configTomlStages_aarch64 // {
        ...
        default = standalonex.packages.${pkgs_aarch64.system}.default.overrideAttrs (oldAttrs: {
          postPatch = ''
            cp ${generatedConfigToml} config.toml
          '';
        });
        ...
        packages.x86_64-linux = configTomlStages_x86_64 // {
          ...
          default = standalonex.packages.${pkgs_x86_64.system}.default.overrideAttrs (oldAttrs: {
            postPatch = ''
              cp ${generatedConfigToml_x86_64} config.toml
            '';
          });
        };

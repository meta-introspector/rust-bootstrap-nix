...
# This script generates a report of the most used types in the CodeGraph.
...
OUTPUT_REPORT_PATH="${PROJECT_ROOT}/.gemini/generated/most_used_types_report.txt"
QUERY_TOOL_BIN="${PROJECT_ROOT}/target/debug/code-graph-query-tool"

# Ensure the output directory exists
mkdir -p "$(dirname "${OUTPUT_REPORT_PATH}")"

echo "Generating 'most-used-types' report..."
"${QUERY_TOOL_BIN}" \
    --graph-path "${CODE_GRAPH_PATH}" \
    --query-type "most-used-types" \

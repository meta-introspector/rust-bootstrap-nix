...
use syn::{visit::Visit, GenericArgument, ItemConst, ItemStruct, Meta, PathArguments, Type};
use walkdir::WalkDir;
...
pub async fn extract_bag_of_types(
    ...
    for entry in WalkDir::new(project_root)
        ...
        .filter(|e| {
            if let Some(names) = filter_names {
                names
                    .iter()
                    .any(|name| e.file_name().to_string_lossy().contains(name))
            } else {
                true
            }
        })

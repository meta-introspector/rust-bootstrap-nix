...
pub fn generate_report(results: &[FileProcessingResult]) -> Result<()> {
    ...
    let successful_files = results
        .iter()
        .filter(|r| matches!(r.status, FileProcessingStatus::Success))
        .count();
    let skipped_files = results
        .iter()
        .filter(|r| matches!(r.status, FileProcessingStatus::Skipped { .. }))
        .count();
    let failed_files = results
        .iter()
        .filter(|r| matches!(r.status, FileProcessingStatus::Failed { .. }))
        .count();
        ...
        for result in results {
            ...
            match &result.status {
                ...
                FileProcessingStatus::Skipped { reason } => {
                    report_content
                        .push_str(&format!("- Status: ⏭️ Skipped (Reason: {}\n\n", reason));
                }

...
use crate::prelude_category_pipeline::{
    AstReconstructionFunctor, ClassifyUsesFunctor, ExtractUsesFunctor,
};
...
pub async fn run_category_pipeline<W: tokio::io::AsyncWriteExt + Unpin + Send>(
    ...
    let parsed_file = parse_functor
        .map(writer, raw_file, _config)
        .await
        .context("Parsing failed")?;
    writer
        .write_all(format!("  -> Parsed file successfully.\n").as_bytes())
        .await?;
    ...
    let use_statements = extract_uses_functor
        .map(writer, parsed_file.clone(), _config)
        .await
        .context("Extracting use statements failed")?;
    writer
        .write_all(
            format!(
                "  -> Extracted {} use statements.\n",
                use_statements.0.len()
            )
            .as_bytes(),
        )
        .await?;
    ...
    let classified_uses = classify_uses_functor
        .map(writer, use_statements, _config)
        .await
        .context("Classifying use statements failed")?;
    writer
        .write_all(format!("  -> Classified use statements:\n").as_bytes())
        .await?;
    writer
    ...
    let reconstructed_code = PipelineFunctor::map(
        &ast_reconstruction_functor,
        writer,
        validated_file.clone(),
        _config,
    )
    .await
    .context("AST Reconstruction failed")?;
    writer
        .write_all(format!("  -> AST Reconstruction completed successfully.\n").as_bytes())
        .await?;

    // Write generated code to a file
    let generated_output_dir = _config
        .as_ref()
    ...
    tokio::fs::write(&output_file_path, reconstructed_code.as_bytes())
        .await
        .context(format!(
    ...
    writer
        .write_all(format!("  -> Generated code written to {:?}\n", output_file_path).as_bytes())
        .await?;
    ...
    writer
        ...
        .await?;
    crate::utils::validate_rust_code(&output_file_path)
        .await
        .context(format!(
            "Generated code validation failed for {:?}",
            output_file_path
        ))?;
    writer
        .write_all(format!("  -> Generated code validated successfully.\n").as_bytes())
        .await?;

    let collected_metrics = measurement::get_collected_metrics();
    let json_metrics = serde_json::to_string_pretty(&collected_metrics)
